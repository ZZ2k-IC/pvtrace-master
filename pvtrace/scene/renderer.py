...
from pvtrace.geometry.plane import Plane
...
class MeshcatRenderer(object):
    ...
    def add_geometry(self, geometry, pathname, transform):
        ...
        if isinstance(geometry, Sphere):
            ...
            # Render Plane as a very thin box for visualization
            
            # Get plane dimensions
            if hasattr(geometry, '_size'):
                width, height = geometry._size
            else:
                width, height = 1.0, 1.0  # Default size
            
            thickness = 0.001  # Very thin for visualization
            
            # Create a thin box mesh
            box_geom = trimesh.creation.box((width, height, thickness))
            
            # Convert to meshcat geometry
            obj = meshcat.geometry.StlMeshGeometry.from_stream(
            ...
            vis[pathname].set_object(obj, detector_material)
            vis[pathname].set_transform(transform)
